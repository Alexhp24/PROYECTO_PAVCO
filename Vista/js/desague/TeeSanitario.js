...
export class TeeSanitario extends SingleShapeBase {
  static labelCount = 0;
  static totalCount = 0;

  constructor(ctx, color) {
    super(ctx, color);
    this.label = `Tee Sanitario Ø4 a Ø3`;
  }
...
/*import { Codo} from "../js/Codo.js";

const verticalLength = 10;
const horizontalLength = 10;

export class TeeSanitario extends Codo{
  static labelCount = 0;
  static totalCount = 0;
  ...
  onDoubleClick(event) {
    ...
    // Comprobar si el clic está dentro del área de dibujado del CodoT
    if (relativeX >= -horizontalLength / 2 && relativeX <= horizontalLength / 2 && relativeY >= -verticalLength / 2 && relativeY <= verticalLength / 2) {
      this.rotation = (this.rotation + 90) % 360; // Incrementar la rotación en 90 grados
      this.draw(); // Volver a dibujar con la nueva rotación
    }
  }
