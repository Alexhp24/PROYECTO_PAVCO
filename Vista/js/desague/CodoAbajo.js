...
export class CodoAbajo extends SingleShapeBase {
  ...
  drawLineWithCircle(x, y) {
    ...
    this.ctx.strokeStyle = "#237DF7"; // Color de borde verde para el círculo más grande
    this.ctx.stroke();
    ...
    this.ctx.strokeStyle = "#237DF7"; // Color negro para la primera línea vertical
    this.ctx.stroke();

    // Dibujar la segunda línea horizontal en la parte superior del círculo
    const horizontalLineStartX = circleX - lineLength / 2; // Izquierda del centro del círculo
    const horizontalLineStartY = circleY - largerCircleRadius - 5; // Justo arriba del círculo
    const horizontalLineEndX = circleX + lineLength / 2; // Derecha del centro del círculo
    const horizontalLineEndY = circleY - largerCircleRadius - 5; // Justo arriba del círculo

    this.ctx.beginPath();
    this.ctx.moveTo(horizontalLineStartX, horizontalLineStartY);
    this.ctx.lineTo(horizontalLineEndX, horizontalLineEndY);
    this.ctx.lineWidth = 2;
    this.ctx.strokeStyle = "#237DF7"; // Color negro para la línea horizontal
    this.ctx.stroke();
